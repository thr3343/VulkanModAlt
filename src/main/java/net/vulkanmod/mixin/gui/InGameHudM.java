...
public abstract class InGameHudM {
     ...
     */
    @Overwrite()
    private void renderPlayerHealth(GuiGraphics poseStack) {
        int ac;
        int ab;
        int aa;
        int z;
        ...
        for (int w = 0; w < 10; ++w) {
            ...
            x = m + w * 8;
            if (w * 2 + 1 < u) {
                poseStack.blit(GUI_ICONS_LOCATION, x, s, 34, 9, 9, 9);
            }
            if (w * 2 + 1 == u) {
                poseStack.blit(GUI_ICONS_LOCATION, x, s, 25, 9, 9, 9);
            }
            if (w * 2 + 1 <= u) continue;
            poseStack.blit(GUI_ICONS_LOCATION, x, s, 16, 9, 9, 9);
        }
        ...
        if (x == 0) {
            ...
            for (y = 0; y < 10; ++y) {
                ...
                ac = n - y * 8 - 9;
                poseStack.blit(GUI_ICONS_LOCATION, ac, z, 16 + ab * 9, 27, 9, 9);
                if (y * 2 + 1 < k) {
                    poseStack.blit(GUI_ICONS_LOCATION, ac, z, aa + 36, 27, 9, 9);
                }
                if (y * 2 + 1 != k) continue;
                poseStack.blit(GUI_ICONS_LOCATION, ac, z, aa + 45, 27, 9, 9);
            }
            t -= 10;
        }
        ...
        if (player.isEyeInFluid(FluidTags.WATER) || z < y) {
            ...
            for (int ad = 0; ad < ab + ac; ++ad) {
                if (ad < ab) {
                    poseStack.blit(GUI_ICONS_LOCATION, n - ad * 8 - 9, t, 16, 18, 9, 9);
                    continue;
                }
                poseStack.blit(GUI_ICONS_LOCATION, n - ad * 8 - 9, t, 25, 18, 9, 9);
            }
        }
     ...
     */
    @Overwrite
    private void renderHeart(GuiGraphics poseStack, Gui.HeartType heartType, int i, int j, int k, boolean bl, boolean bl2) {
        poseStack.blit(GUI_ICONS_LOCATION, i, j, heartType.getX(bl2, bl), k, 9, 9);
    }
}

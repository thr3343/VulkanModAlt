...
//                SPIRV fragShaderSPIRV = compileShader(path + ".fsh", "fragment");
        try (MemoryStack stack = MemoryStack.stackPush()) {
            long compShaderSPIRV = ShaderSPIRVUtils.compileShaderFile(path + ".csh", ShaderSPIRVUtils.ShaderKind.COMPUTE_SHADER, "/assets/vulkanmod/shaders/post/");

            compShaderModule = Pipeline.createShaderModule(compShaderSPIRV);
            ...
            if(vkCreateComputePipelines(device, Pipeline.PIPELINE_CACHE, pipelineInfo, null, pGraphicsPipeline) != VK_SUCCESS) {
...
//        bufferInfos.buffer(ssboStorageBuffer);
        ...
        imageInfos.imageView(Drawer.tstFrameBuffer2.getColorAttachment().getImageView());

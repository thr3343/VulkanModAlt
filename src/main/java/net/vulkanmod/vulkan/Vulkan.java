...
//            Validate.isTrue(imageCount.get(0) > 5);
    ...
    private static void createRenderPass() {
        ...
        try(MemoryStack stack = stackPush()) {
            ...
            depthAttachment.finalLayout(VK12.VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL);

            VkAttachmentReference depthAttachmentRef = attachmentRefs.get(1);
            depthAttachmentRef.attachment(1);
            depthAttachmentRef.layout(VK12.VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL);
    ...
    private static void createDepthResources() {
        ...
        try(MemoryStack stack = stackPush()) {
            ...
            transitionImageLayout(commandBuffer, depthImage, depthFormat,
                    VK_IMAGE_LAYOUT_UNDEFINED, VK12.VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL, 1);
            endImmediateCmd();
    ...
    //Use the fastest Depth Only format depending on the GPU (Nvidia seems to prefer 24-Bit or lower, while AMD performs best with 32-Bit Float)
    ...
    public static long createImageView(long image, int format, int aspectFlags, int mipLevels) {
        ...
        try(MemoryStack stack = stackPush()) {
            ...
            VkImageViewCreateInfo viewInfo = VkImageViewCreateInfo.calloc(stack)
                    ...
                    .format(format);
            viewInfo.subresourceRange().set(aspectFlags, 0, mipLevels, 0, 1);
